/*global history */
sap.ui.define([
	"assetinv/AssetInventory/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"sap/ui/model/Filter",
	"sap/ui/model/FilterOperator",
	"sap/m/GroupHeaderListItem",
	"sap/ui/Device",
	"assetinv/AssetInventory/model/formatter",
	"sap/m/MessageBox"
], function(BaseController, JSONModel, Filter, FilterOperator, GroupHeaderListItem, Device, formatter, MessageBox) {
	"use strict";

	return BaseController.extend("assetinv.AssetInventory.controller.Master", {
		...
		onInit: function() {
			...
			this._oListSelector = this.getOwnerComponent()
				.oListSelector;
			...
			this._oODataModel = this.getOwnerComponent()
				.getModel();
		},
		...
		handleIconTabBarSelect: function(oEvent) {
			...
			if (sKey === "All") {
				//Need to Clear the filter in this case
			} else if (sKey === "Open") {
				aFilters.push(new Filter("ZAlctd", FilterOperator.EQ, "" ));
			} else if (sKey === "Located") {
				aFilters.push(new Filter("ZAlctd", FilterOperator.EQ, "Y" ));
			}
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("Txt50", FilterOperator.Contains, sQuery)];
			} else {
		...
		_leaveEditPage: function(fnLeave, fnLeaveCancelled) {
			var sQuestion = this.getResourceBundle()
				.getText("warningConfirm");
			...
			this._oListSelector.oWhenListLoadingIsDone.then(
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						...
						var sObjectPath = mParams.firstListitem.getBindingContext()
							.getPath();
						this.getRouter()
							.getTargets()
							.display("change", {
						...
						/*						this.getRouter().navTo("object", {
													Anln1: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("Anln1"))
												}, true);
						*/
					}
				}.bind(this),
				function(mParams) {
					...
					this.getRouter()
						.getTargets()
						.display("detailNoObjectsAvailable");
				}.bind(this)
			);
		},
		...
		_showDetail: function(oItem) {
			...
			var sObjectPath = oItem.getBindingContextPath(); // this.getView().getElementBinding().getPath();
			this.getRouter()
				.getTargets()
				.display("change", {
					mode: "update",
			...
			/*			var bReplace = !Device.system.phone;
						this.getRouter().navTo("create", {
							Anln1: encodeURIComponent(oItem.getBindingContext().getProperty("Anln1"))
						}, bReplace);
			*/
		...
		_updateListItemCount: function(iTotalItems) {
			...
			if (this._oList.getBinding("items")
				...
				this.getModel("masterView")
					.setProperty("/totalRecords", iTotalItems);
		 ...
		 */
		_findItem: function() {
			var itemToSelect = this.getModel("appView")
				.getProperty("/itemToSelect");
			if (itemToSelect) {
